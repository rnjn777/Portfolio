          ...
          >
            <form onSubmit={handleSubmit} className="space-y-8 flex flex-col h-full justify-between">
              ...
              <button
                ...
                className="magnetic mt-12 w-full py-6 bg-[var(--text-primary)] text-[var(--bg-void)] font-[family-name:var(--font-mono)] font-bold tracking-widest uppercase hover:bg-[var(--accent-cyan)] transition-colors flex justify-center items-center gap-4 group disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {status === "loading" ? "TRANSMITTING..." : 
                 status === "success" ? "MESSAGE RECEIVED" : 
                 status === "error" ? "TRANSMISSION FAILED (TRY AGAIN)" : 
                 "SEND MESSAGE"}
                {status === "idle" && <ArrowRight size={16} className="group-hover:translate-x-2 transition-transform" />}
              </button>
            </form>
          </motion.div>
        </div>

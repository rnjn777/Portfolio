...
import HoverGlitchText from "@/components/ui/HoverGlitchText";
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 relative">
                    ...
                    <div className="absolute -left-4 sm:-left-8 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-[var(--accent-cyan)] opacity-0 group-hover:opacity-100 transition-opacity duration-300 shadow-[0_0_10px_var(--accent-cyan)]" />
                    ...
                    <h3 className={`text-2xl sm:text-4xl lg:text-5xl font-bold uppercase tracking-tight transition-all duration-500 leading-tight ${
                      isExpanded 
                        ...
                        : "text-[var(--text-primary)] group-hover:translate-x-4"
                    }`}>
                      {isExpanded ? (
                        item.role
                      ) : (
                        <HoverGlitchText text={item.role} />
                      )}
                    </h3>
                  </div>
                  ...
                  {isExpanded && (
                    ...
                    >
                      <div className="pb-10 sm:pb-14 pl-14 sm:pl-[4.5rem] flex flex-col lg:flex-row gap-8 lg:gap-16">
                        ...
                        {/* Right: Description */}
                        <div className="flex-1 border-l-2 border-[var(--border-glass)] pl-6 sm:pl-10 relative">
                          <div className={`absolute top-0 bottom-0 left-[-2px] w-[2px] bg-[var(--accent-cyan)] transition-all duration-1000 origin-top ${isExpanded ? 'scale-y-100 opacity-100' : 'scale-y-0 opacity-0'}`} />
                          <span className="font-[family-name:var(--font-mono)] text-[10px] text-[var(--text-muted)] uppercase tracking-widest block mb-4">
                            Mission Brief
                          </span>
                          ...
                          <div className="mt-8 font-[family-name:var(--font-mono)] text-[10px] text-[var(--text-muted)] bg-[var(--bg-glass)] p-3 rounded border border-[var(--border-glass)] inline-block">
                            <span className="text-[var(--accent-cyan)]">&gt;</span> status: {isWork ? "COMPLETED" : "ACTIVE"} 
                            <span className="ml-2 inline-block w-2 h-3 bg-[var(--accent-cyan)] animate-pulse shadow-[0_0_8px_var(--accent-cyan)] align-middle" />
                          </div>
                        </div>
                      </div>
                    </motion.div>
